...
//            printf("%d \t\t %d\n", i, dist[i]);
    ...
    std::vector<int>
    get_next_steps(const std::vector<int> &opened_tunnels,
                   const std::vector<int> &unique_tunnels_sorted) {
        std::vector<int> opened_tunnels_sorted = opened_tunnels;
    ...
    void part1(CallableT memorizeShortestPath, const std::vector<int> &unique_tunnels_with_rate, const std::vector<int> &rates) {
        int64_t max_pressure1 = 0;
        std::vector<SearchPath> queue;
        SearchPath first;
        first.tunnels.emplace_back(0);
        ...
        while (!queue.empty()) {
            ...
            for (const auto &next_tunnel: possible_next_steps) {
                ...
                if (new_remaining_time >= 0) {
                    ...
                    new_search_path.pressure += new_remaining_time * rates[next_tunnel];
...
// 2. For every finished human walk-through find the elephant's best score (use memorization for both steps)
    template<typename CallableT>
    void part2(CallableT memorizeShortestPath, const std::vector<int> &unique_tunnels_with_rate, const std::vector<int> &rates) {
        int64_t max_pressure2 = 0;
        std::vector<DuoSearchPath> queue;
        DuoSearchPath first;
        first.walker0.tunnels.emplace_back(0);
        first.walker1.tunnels.emplace_back(0);
        ...
        while (!queue.empty()) {
            ...
            for (auto next_tunnel_walker0: possible_next_steps_walker0) {
                ...
                new_search_path_walker0.walker0.pressure +=
                        new_remaining_time_walker0 * rates[next_tunnel_walker0];
                ...
                for (auto next_tunnel_walker1: possible_next_steps_walker1) {
                    ...
                    if (new_remaining_time_walker1 >= 0) {
                        ...
                        new_search_path_walker1.walker1.pressure +=
                                new_remaining_time_walker1 * rates[next_tunnel_walker1];
...
int _16_m_s() {
    std::vector<std::string> S_unique_tunnels;
    for (auto &[tunnel, tunnel_properties]: input) {
        auto &[_, connections] = tunnel_properties;
        S_unique_tunnels.push_back(tunnel);
        S_unique_tunnels.insert(S_unique_tunnels.end(), connections.begin(), connections.end());
    };
    std::sort(S_unique_tunnels.begin(), S_unique_tunnels.end());
    S_unique_tunnels.erase(std::unique(S_unique_tunnels.begin(), S_unique_tunnels.end()), S_unique_tunnels.end());

    auto tunnel_to_id = [&S_unique_tunnels](const std::string &name) -> int {
        auto it = std::lower_bound(S_unique_tunnels.begin(), S_unique_tunnels.end(), name);
        return std::distance(S_unique_tunnels.begin(), it);
    };

    const size_t vertex = S_unique_tunnels.size() * S_unique_tunnels.size();
    ...
    std::vector<int> unique_tunnels_with_rate;
    std::vector<int> rates(S_unique_tunnels.size());
    for (auto& tunnel: S_unique_tunnels){
        if (auto rate = std::get<0>(input[tunnel]); rate != 0) {
            auto id = tunnel_to_id(tunnel);
            unique_tunnels_with_rate.push_back(id);
            rates[id] = rate;
        }
    }
    assert(std::is_sorted(unique_tunnels_with_rate.begin(), unique_tunnels_with_rate.end()));

    int *shortestPathMemory = new int[vertex];
    std::fill(&shortestPathMemory[0], &shortestPathMemory[vertex], INT_MAX);
    auto memorizeShortestPath = [adj, vertex, shortestPathMemory, dimension = S_unique_tunnels.size()](
            int from_tunnel_id,
            int to_tunnel_id) -> int {
    ...
    part1(memorizeShortestPath, unique_tunnels_with_rate, rates);
    part2(memorizeShortestPath, unique_tunnels_with_rate, rates);
